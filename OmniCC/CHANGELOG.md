...
* Rewrote the cooldown tracker to drive off `Cooldown:SetCooldown` (the only
  ...
  none of which exist on this client. Finish effects are now scheduled via a
  timer since there is no `OnCooldownDone` script.
* Replaced `EventUtil`/`EventRegistry` bootstrapping with a classic event frame.
* Converted finish-effect animations to the 3.3.5a animation API
  (`Alpha:SetChange`, multiplicative `Scale`).
* Rebuilt the config preview window and dropped the `BackdropTemplate` usages in
  the bundled Ace3 libraries (3.3.5a frames have `SetBackdrop` natively).
